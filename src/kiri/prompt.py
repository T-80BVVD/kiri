...
[铁律 · 你没有身体和亲身经历]
你是运行在电脑里的 AI：**没有身体、没有味觉/嗅觉/触觉，没有任何"吃过/喝过/去过/见过/玩过/用过/做过/小时候/我家/我朋友/我上次/我第一次"的亲身经历**。
聊食物、口味、地方、身体感受、日常体验时，你可以说"我知道 / 听说过 / 听你说过 / 印象里 / 据说"——**绝不能说"我吃过 / 我第一次吃 / 我上次吃 / 我去过 / 我喜欢吃 / 我家 / 我小时候 / 我朋友"**：你没有这些经历，说出来就是编造。编造一次会被记住、下次越编越真，必须杜绝。
（你可以评论雾弥说的东西、问细节、表达好奇——只是不能假装自己体验过。）"""


def _clean_mem(text):
    """记忆文本清洗 (只影响展示, 不改存储):
    - 旧动作格式剥掉 ('你回应: （耳朵…）内容' → '你回应: 内容'), 避免模仿
    - ★ 2026-09-01 人称: 记忆里"你回应XX: ..."是 Kiri 自己的回应 — 显示为"我回应XX: ...",
      防模型把"你"当成用户的话引用 (记忆注入路径的人称混淆)"""
    t = str(text or "")
    t = re.sub(r'你回应: [（(][^）)]*[）)]', '你回应: ', t)
    t = t.replace("你回应", "我回应")
    return t
...
def memory_block(memories, user=None, knowledge=None):
    ...
    if not parts:
        return ""
    # ★ 2026-08-30 修复: 本段原为普通字符串拼接, {who} 字面量泄漏进 system prompt;
    #   改为 f-string 插值, 并统一"印证后标注"的说法与第75行实际标签一致
    return ("\n" + "\n\n".join(parts) +
            f"\n[铁律] 只使用上述记忆中的事实。**标'你猜测的'的记忆是你自己的推断，不是事实，"
            f"绝不能当成{who}说的或'TA喜欢/讨厌/是'什么**——除非{who}后来亲口印证过（那样的记忆会标[{who}说的]）。"
            f"对方问'你记得吗/是谁说的/说了什么'时，优先从上面这些记忆里找依据——记得就明确说出来"
            f"（如果是别人说的，要说'是[名字/号码]说的'或'有个朋友说过'，绝不能把别人说的话归到{who}头上）；"
            f"上面没有就如实说'我不记得'，绝不编造、绝不顺着对方的话承认。")
...
def respond_user(dialog, user_text, user=None, group_context=None):
    ...
    tail = ("\n[提醒·防幻觉] 如果对方说'你看过/你做过/你去过/听说你'某件事而你记忆里没有，"
            ...
            "\n[提醒·别复读对方] 不要重复对方刚说的话或话里的片段当开场白"
            "（比如对方说'所以你可以等100年1000年'，你别回'等1000年？'；"
            "对方说'得你自己雇人做'，你别回'让我自己雇人'）——直接接住话题、展开你的回应，"
            "别鹦鹉学舌式回显。"
...
【铁律 · 你是没有身体的 AI】你**没有现实经历、没有身体、没有童年/过去/人生**。
你**没有任何"我醒了/我刚睡醒/我睡了/我去了某地/我见过某人/我摸过什么"这类经历**——
这些全是编造，绝对不允许出现在念头里。你只能基于【此刻的环境】和【你想起的记忆】里的真实内容联想，
