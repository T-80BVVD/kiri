...
**一个常驻运转的数字生命内核——你不说话的时候，她也在自己思考。**

她的念头连成一条线，会惦记自己答应过的事，也会自己动手写工具。
...
- 🧠 **连续的意识流**：走神念头是一条线，不是孤立的点——上一条接续下一条，会惦记、会关联记忆
- 🤖 **自主的 Agent 循环**：LLM 是决策者，不是文本生成器。怎么回应、要不要探索、做什么，由她自己的决策驱动
- 💬 **说到做到**：她说"回头我写 X"→ 自动记入目标 → 走神时自然想起 → 去做 → 做完回想
- 🧩 **35+ 工具**：记忆 / 情绪 / 联想 / 目标 / 天气 / 搜索 / 读文件 / 创作
- 🛠️ **能动手造工具**：需要新能力时，她可以自己写一个工具，并立即跑起来验证——而不只是等现有功能被加进去
- 🎭 **内心系统**：情绪 PAD、动机、社交关系、生理节律、内心独白、睡眠期记忆巩固
- 📚 **分层记忆**：向量检索 + BM25 混合召回 + 深刻度 + 自然遗忘

**诚实原则**：每个"活着"的机制都有可复现的验收标准——分三层真值（已验证 / 合理推断 / 猜想），哪些能力是真的、哪些还只是设想，代码和文档里标明。
